...
public class PlantViewModel extends ViewModel {
    ...
    private final DatabaseReference idsReference;
    ...
    public PlantViewModel() {
        databaseReference = FirebaseDatabase.getInstance().getReference("plants"); // Collection of plant nodes under root "plants"
        idsReference = FirebaseDatabase.getInstance().getReference("plant_ids"); // Pool of ids generated by ESP32 devices
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot plantSnapshot : snapshot.getChildren()) { //Every plant captured here is a child of the "plants" node which is the root node.
                    ...
                    // Display name is stored as a field now, since the key is a plant_id (e.g. from the ESP32's MAC address)
                    ...
                    // Note: key (plant_id) is used internally for all database operations;
                    // name is only for display. PlantReading stores the id as its identifying "name"
                    // field so requestManualWatering/deletePlant/etc keep working unchanged.
    ...
    /**
     * Callback used by the UI to report whether an id slot is available,
    ...
    /**
     * Checks whether at least one untaken plant_id exists, without claiming it.
    ...
    public void checkIdAvailability(AvailabilityCallback callback) {
        idsReference.get().addOnSuccessListener(snapshot -> {
            for (DataSnapshot idSnap : snapshot.getChildren()) {
                Boolean taken = idSnap.child("taken").getValue(Boolean.class);
                if (taken == null || !taken) {
    ...
    /**
     * Claims the first available plant_id and creates a new plant profile under it.
     * Replaces the old behavior of writing directly to /plants/{userTypedName}.
     */
    ...
    public void addPlant(String plantName, AddPlantCallback callback) {
        idsReference.get().addOnSuccessListener(snapshot -> {
            ...
            claimIdAndCreatePlant(candidateId, plantName, callback);
        }).addOnFailureListener(e -> {
            if (callback != null) callback.onNoSlotsAvailable();
        });
    }

    private void claimIdAndCreatePlant(String plantId, String plantName, AddPlantCallback callback) {
        DatabaseReference takenRef = idsReference.child(plantId).child("taken");

        takenRef.runTransaction(new Transaction.Handler() {
            @NonNull
            @Override
            public Transaction.Result doTransaction(@NonNull MutableData currentData) {
                Boolean current = currentData.getValue(Boolean.class);
            ...
            public void onComplete(DatabaseError error, boolean committed, DataSnapshot snapshot) {
                if (!committed) {
                    // Lost the race on this id — retry with the next available one
                    addPlant(plantName, callback);
                    return;
                }

                DatabaseReference newPlantRef = databaseReference.child(plantId);

                newPlantRef.child("name").setValue(plantName);
    ...
    /**
     * Deletes the plant and frees its plant_id slot so the ESP32 device
     * can be reassigned to a new plant profile later.
     */
    public void deletePlant(String plantId) {
        databaseReference.child(plantId).removeValue();
        idsReference.child(plantId).child("taken").setValue(false);
    }
}