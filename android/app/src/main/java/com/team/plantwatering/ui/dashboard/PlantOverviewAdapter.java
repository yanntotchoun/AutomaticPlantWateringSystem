...
public class PlantOverviewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_INTRO) return new IntroViewHolder(inflater.inflate(R.layout.item_summary_card, parent, false));
        return new PlantRowViewHolder(inflater.inflate(R.layout.item_plant_overview_row, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof IntroViewHolder) ((IntroViewHolder) holder).bind(plants);
        else ((PlantRowViewHolder) holder).bind(plants.get(position - 1), clickListener, currentServerTime);
    }
    ...
    static class IntroViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(List<PlantReading> plants) {
            ...
            for (PlantReading plant : plants) {
                if (plant.getSoilHumidity() < settingsManager.getThresholdProfile(plant.getThresholdId()).drySoil) dryPlants++;
            }
    ...
    static class PlantRowViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(PlantReading plant, PlantClickListener clickListener, long currentServerTime) {
            ...
            humidityTank.setText(String.format(Locale.getDefault(), "Humidity: %d%%  |  Tank: %s", plant.getSoilHumidity(), plant.getWaterTank()));
